...
def run_as_admin(command, args=None,capture=False):
    try:
        if args is None:
            args = []
        ps_args = f'"{command}" ' + ' '.join([f'"{arg}"' for arg in args])
        ps_command = f'Start-Process -Verb RunAs -FilePath {ps_args}'
...
class CFWarpGUI:
    ...
    # def select_csv(self, event):
    #     self.filename = tkinter.filedialog.askopenfilename(title="选择csv文件", filetypes=[("csv文件", "*.csv")])
    ...
    def wg_direct(self, event):
        ...
        if is_admin():
            ...
            run_as_admin("wireguard ",["/installtunnelservice {}".format(dir_path+"\\wg-node.conf")])
